...
const Home = () => {
  const isCategoryLoading = useSelector(
    (state) => state.product.isCategoryLoading
  );
  ...
  return (
    <>
      <Helmet>
        <title>
          GroceMart: Groceries in Minutes - A bold promise that can attract
          time-crunched customers.
        </title>
        <meta
          name="description"
          content="Buy fresh groceries online at competitive prices. Fast delivery, quality produce, and weekly specials."
        />
      </Helmet>
      <div className="p-2 lg:px-20 md:py-10">
        <div className="w-full mb-2 sm:mb-8 ">
          <img
            ...
            src="https://res.cloudinary.com/shivamka/image/upload/v1750051526/5423986_wae2ly.jpg"
            alt="Fresh groceries"
          />
          <img
            draggable={false}
            className="block sm:hidden w-full rounded-lg"
            src="https://res.cloudinary.com/shivamka/image/upload/v1750051569/56027859164_ko3ibe.png"
            alt="Fresh groceries"
          />
        </div>
        {isCategoryLoading ? (
        ...
        {allCategory[0]
          ? allCategory.map((cat) => (
              <ProductByCategory
                ...
                navigateTo={ProductListPageUrl(cat)}
              />
            ))
          : Array(4)
              .fill(null)
